...
function renderSettingsPage() {
  ...
  return `
    <div class="page settings-page">
      ...
      <div class="settings-section open" style="border-color:#FECACA;">
        <button type="button" class="settings-section-head" onclick="this.parentElement.classList.toggle('open')">
          <span>Reset everything</span>
          <span class="collapse-chevron">▾</span>
        </button>
        <div class="settings-section-body">
          <p class="settings-desc">Clears the plan in this browser, all LifePlan local data, and restores factory defaults (as if you opened the app for the first time). Use when testing, or if you want a completely clean start. This cannot be undone — save a <code>.lifeplan.json</code> file first if you need a copy.</p>
          <button type="button" class="btn-secondary" style="color:#DC2626;border-color:#FECACA;" onclick="factoryResetApp()">Reset to defaults…</button>
        </div>
      </div>
    </div>
  `;
...
async function factoryResetApp() {
  const ok = await appConfirmYesNo(
    "Reset everything to defaults?\n\n• Deletes the current plan in this browser\n• Clears LifePlan local storage\n• Starts as a first-time user (wizard can run again)\n\nThis cannot be undone. Save a file first if you need the plan.",
    "Reset everything",
    "Cancel"
  );
  if (!ok) return;
  try {
    ...
    for (let i = 0; i < localStorage.length; i++) {
      ...
      if (k && (k.startsWith("lifeplan") || k.startsWith("LifePlan"))) keys.push(k);
    }
    keys.forEach(k => localStorage.removeItem(k));
    // Known keys in case the loop misses anything
    ["lifeplan_current", "lifeplan_welcomed", "lifeplan_undo", "lifeplan_redo"].forEach(k => {
  ...
  if (typeof undoStack !== "undefined") {
    try { window._undoStack = []; window._redoStack = []; } catch (e) {}
  }
  autoSave();
  updateSidebarPlanLabel();
  updateSidebarUser();
  await appAlert("Everything has been reset. The page will reload.");
  location.reload();
}
...
/** First paragraph of a help node as plain HTML (for tour blurbs). */
function helpPurposeBlurb(helpId) {
  try {
    const node = typeof findHelpNode === "function" ? findHelpNode(helpId) : null;
    if (!node || !node.body) return "";
    const m = String(node.body).match(/<p[^>]*>([\s\S]*?)<\/p>/i);
    return m ? m[1].trim() : "";
  } catch (e) { return ""; }
}

/**
 * Show-me-around tour. Prefer `message`; else Help opening paragraph.
 * TODO: remove inDev banners when Plans / Strategy advanced features ship.
 */
const WIZARD_TOUR = [
  {
    id: "dashboard", page: "dashboard", helpId: "dashboard", label: "Dashboard",
    message: "Once your details are in, this is home base. Use <strong>Solve balance</strong> here to work out a sustainable <strong>monthly spend</strong> and check the gauge."
  },
  { id: "people", page: "people", helpId: "people", label: "People" },
  { id: "accounts", page: "accounts", helpId: "accounts", label: "Accounts" },
  { id: "income", page: "income", helpId: "income", label: "Income" },
  {
    id: "spend", page: "spend", helpId: "spend", label: "Spend",
    message: "Here you design a <strong>template</strong> of how you’d like to spend each month — not the final answer. LifePlan uses that shape (and the bandwidth) with your pots and income to <strong>calculate</strong> the monthly spend figure."
  },
  {
    id: "strategy", page: "strategy", helpId: "strategy", label: "Strategy",
    inDev: true,
    message: "A <strong>default strategy</strong> is already in place so money can move through savings and the model can run. More controls are coming; you can leave this as it is for now."
  },
  {
    id: "plans", page: "plans", label: "Plans",
    inDev: true,
    message: "This area will hold templates and multiple scenarios. <strong>Right now you work on one plan at a time</strong> in this browser (use Save / Save as for copies)."
  },
  {
    id: "reports", page: "reports", label: "Reports",
    message: "Printable summaries of your plan — for example the annual report and the figures table — so you can review numbers away from the screen."
  },
  {
    id: "assumptions", page: "assumptions", label: "Assumptions",
    message: "Shared defaults the model can use: inflation, simple tax bands, and state pension figures. Change these when you want different starting assumptions."
  },
  { id: "settings", page: "settings", helpId: "settings", label: "Settings" },
  { id: "help", page: "dashboard", helpId: "know", label: "Help", openHelp: true },
    ...
    <div class="wizard-step wizard-welcome">
      ...
      <div class="wizard-choice-grid">
        ...
        <button type="button" class="wizard-choice" onclick="wizardChoosePath('tour')">
          ...
          <span>Short tour of each main screen (same ideas as Help)</span>
        </button>
...
function renderWizardTour() {
  ...
  if (step.id === "done") {
    ...
    body.innerHTML = `
      <div class="wizard-step">
        ...
        <p class="wizard-lead">Use the <strong>Dashboard</strong> for monthly spend and the balance gauge. Open <strong>Help</strong> anytime for the same short explanations you just saw — especially <em>Things you should know</em>.</p>
        <div class="wizard-actions">
  ...
  if (step.page) goToPage(step.page);
  if (step.openHelp && typeof openHelp === "function") {
    setTimeout(() => { try { openHelp(step.helpId || "know"); } catch (e) {} }, 80);
  } else {
    setTimeout(() => highlightNav(step.page), 50);
  }

  const title = step.label || step.title || "Tour";
  const blurb = step.message
    || ((step.helpId && typeof helpPurposeBlurb === "function") ? helpPurposeBlurb(step.helpId) : "")
    || "This section is covered in Help.";
  const tip = step.openHelp
    ? "Help is open on the matching topic. Close it and continue when ready."
    : "The menu item for this section is highlighted. Click around if you like, then Continue.";
  const inDev = step.inDev
    ? `<p class="wizard-indev"><strong>In development</strong> — features here are still being built.</p>`
    : "";
  ...
  body.innerHTML = `
    <div class="wizard-coach">
      ...
      <h3>${title}</h3>
      ${inDev}
      <p class="wizard-lead">${blurb}</p>
      <p class="field-hint" style="margin-top:6px;">${tip}</p>
      <div class="wizard-actions">
  ...
  if (step.id === "horizon") {
    ...
    body.innerHTML = `
      <div class="wizard-step">
        ...
        <p class="wizard-lead">Defaults go to when the youngest person turns 100 — change them if you want a shorter picture.</p>
        ...
        <div class="wizard-done-box" style="margin-bottom:14px;">
          ...
          <p style="margin:0 0 8px;font-size:13px;color:#475569;line-height:1.45;">This is simply the <strong>last year on the horizontal axis</strong> of your graphs and reports — how far the timeline is drawn. It does not by itself decide how long savings support your spending.</p>
          <div class="form-group" style="margin:0;">
        ...
        <div class="wizard-done-box" style="margin-bottom:14px;">
          <strong>Savings target year</strong>
          ...
          <p style="margin:0 0 8px;font-size:13px;color:#475569;line-height:1.45;">Think of the next two boxes together: <strong>“In this year, I want to be left with this much savings.”</strong> LifePlan shapes monthly spend so that, at that year, pots are aimed at your minimum (not run forever past it). After that year, spend is matched to income only.</p>
          <div class="form-group" style="margin:0 0 12px;">
            <label>Fund until (the year) <span class="field-hint" id="wizFundAgeInline">${ageFund ? "(" + escapeHtml(ageFund) + ")" : ""}</span></label>
            <input type="number" id="wizFundUntil" min="${startY}" value="${fundY}">
          </div>
          <div class="form-group" style="margin:0;">
            <label>Minimum savings left then (£)</label>
            <input type="number" id="wizMinNW" min="0" step="1000" value="${minNW}">
            <p class="field-hint" style="margin-top:6px;">How much you want still in the pot at that year. <strong>0</strong> means you’re happy for savings to be used up; try 10,000 or 20,000 if you always want a cushion.</p>
          </div>
        </div>
...
function renderWizardBuildDone() {
  ...
  body.innerHTML = `
    <div class="wizard-step">
      ...
      <p class="wizard-lead">Here's what we set up for <strong>${escapeHtml(name)}</strong>${partner ? ` &amp; <strong>${escapeHtml(partner)}</strong>` : ""}. Treat every figure as a starting point — change anything that doesn't look right. A short <strong>Show me around</strong> tour is recommended next.</p>
      ...
      <div class="wizard-done-box">
        ...
        <ol style="margin:8px 0 0 1.1rem;padding:0;">
          <li>Optional: <strong>Show me around</strong> — a short tour of each main screen (same ideas as Help)</li>
          <li>Read <strong>Help → Things you should know</strong> — gauge, Solve balance, today’s money</li>
          <li>Open the <strong>Dashboard</strong> — check monthly spend and the balance gauge</li>
          <li>Add more accounts or incomes if you need them; reshape spend or run Solve balance when ready</li>
        </ol>
      </div>
      <div class="wizard-actions" style="flex-wrap:wrap;">
        <button type="button" class="btn-primary" onclick="wizardChoosePath('tour')">Show me around</button>
        <button type="button" class="btn-secondary" onclick="completeWizard()">Go to dashboard</button>
        <button type="button" class="btn-secondary" onclick="completeWizard();openHelp('know');">Things you should know</button>
